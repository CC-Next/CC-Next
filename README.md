...
If your download's commit ID doesn't match the branch it's built from then wait a while and download again, the builds can take a little while (> 8 mins for some) and will only take longer as more PRs get added. It also seems that nightly.link can take a moment to notice that there are new builds, I belive that logged in users can download [direct from Github](https://github.com/CC-Next/CC-Next/actions/workflows/update-and-build.yml).
